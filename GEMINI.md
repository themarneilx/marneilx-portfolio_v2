# Project: Personal Portfolio v2 (Next.js Migration)

## 📄 Source Material

**Primary Task:** Read and parse the provided `index.html` file. This file contains all the content, structure, and text for the portfolio. The new Next.js components should be based directly on the sections found in this file.

* **File to Read:** `index.html`
* **Action:** Migrate all sections (Hero, About, Skills, Projects, Contact) from this HTML file into their corresponding `.tsx` components.
...
1.  **Component Migration:** Convert the static HTML sections from `index.html` into reusable React components. The primary sections to build are:
    * `Navbar`
    * `Hero` (Home)
    * `About`
    * `Skills`
    ...
    * `ContactForm`
    * `Footer`
2.  **UI with DaisyUI:** Re-style the entire site using **DaisyUI components** to create a "beautiful UI." We will use components like `card`, `navbar`, `btn`, `hero`, `drawer`, etc.
3.  **GSAP Animations:** Implement "nice animations" for specific components:
    * **Hero Section:** Recreate the intro and float animations using GSAP.
    * **Project Cards:** Add scroll-triggered or hover animations to the project cards.

## 🛠️ Available MCP Tools

* **`context7`**:
    * **Use this tool** to fetch the latest documentation and best practices for:
        * Next.js (App Router, components, etc.)
        * React (hooks, state management)
        * DaisyUI (component usage, theming)
        * Tailwind CSS (utility classes)
* **`gsap-master`**:
    * **Use this tool** to generate all GSAP-related code.
    * This is the primary tool for creating animations for the **hero section** and **project cards**.
    * Ensure generated code uses the `useGSAP()` hook for proper React integration and cleanup.
...
* `/` (root)
    * `app/`
        * `layout.tsx`
        * `page.tsx` (Main page, will import all section components)
        * `components/`
            * `Navbar.tsx`
            * `Hero.tsx`
            * `About.tsx`
    ...
    * `public/`
        ...
        * `library.png`
        * `cv.pdf`
    * `index.html` (The source HTML file to read)
    * `tailwind.config.ts` (This file will load DaisyUI as a plugin)
    * `GEMINI.MD` (This file)

## Development Notes

* All components should be written in **TypeScript (.tsx)**.
* Prioritize using DaisyUI's pre-built classes (e.g., `card`, `btn-primary`) over custom CSS to maintain a consistent design.
* The old portfolio's `style.css` (like `animate-float`) will be replaced entirely by GSAP animations.

# CONTEXT FOR DAISYUI

This is an llms.txt file for daisyUI.
daisyUI is a component library for Tailwind CSS.
It provides component classes to Tailwind CSS.
